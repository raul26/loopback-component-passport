...
module.exports = function(UserCredential) {
  ...
  UserCredential.link = function (userId, provider, authScheme, profile,
    ...
    var query = { provider: provider, externalId: profile.id, accountId: userId };
    // query[userCredentialModel.relations.user.keyFrom]= user.id;
    console.log('User credential query', query);
