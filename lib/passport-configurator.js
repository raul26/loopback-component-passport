...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  var defaultCallback = function(req, res, next) {
    ...
    passport.authenticate(name, _.defaults({session: session},
      ...
      if (user === '404') {
        var redirect = failureRedirect + '?code=' + encodeURIComponent('404');
      ...
      if (session) {
        req.logIn(user, function(err) {
          ...
          if (info && info.accessToken) {
            if (!!options.json) {
              /*
              * Instead return a JSon I'll send query params.
              */
              /*
              return res.json({
              ...
              */
              var query = '?accountId=' + user.id;
              var redirectPath = successRedirect(req);
              var redirect = redirectPath + query;
              return res.redirect(redirect);
            } else {
